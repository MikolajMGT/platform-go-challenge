...
func NewCassandraRepo(logger logging.Logger, session *gocql.Session) (repo *CassandraRepo) {
	...
	if err := session.Query(CreateTableQuery()).WithContext(ctx).Exec(); err != nil {
		panic(errors.Wrap(err, "failed to inspect/create audiences table"))
	}

	return &CassandraRepo{logger: logger, session: session}
}

func (cr *CassandraRepo) Select(ctx context.Context, params ports.SelectAudiencesRepoParams) (results []assets_dm.AudienceEntity, next string, err error) {

	cr.logger.Info("audiences_db.Select() performed",
		"params", params,
		"results", results,
	)
	...
	var obj assets_dm.AudienceEntity

	scanner := iter.Scanner()
	for scanner.Next() {
		if err = scanner.Scan(&obj.Id, &obj.AgeGroup, &obj.BirthCountry, &obj.CreateTime, &obj.Gender, &obj.PurchasesLastMonth, &obj.SocialMediaHours, &obj.UpdateTime); err != nil {
			return nil, next, err
		} else {
...
func (cr *CassandraRepo) Insert(ctx context.Context, models ...assets_dm.AudienceEntity) (results []assets_dm.AudienceEntity, err error) {

	cr.logger.Info("audiences_db.Insert() performed",
		"params", models,
		"results", results,
	)
...
func (cr *CassandraRepo) Delete(ctx context.Context, models ...assets_dm.AudienceEntity) (results []assets_dm.AudienceEntity, err error) {

	cr.logger.Info("audiences_db.Delete() performed",
		"params", models,
		"results", results,
	)
...
func (cr *CassandraRepo) execute(ctx context.Context, models []assets_dm.AudienceEntity, action func(batch *gocql.Batch, asset assets_dm.AudienceEntity)) (err error) {
